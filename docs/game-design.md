...
Standard summon використовує відкриті odds `Common 60% · Rare 28% · Epic 10% · Legendary 2%`. Snapshot schema v7 зберігає server-authoritative Legendary pity: після 59 summon без Legendary шістдесятий примусово вибирає Legendary template, після чого counter скидається. Новий character отримує 30 gems на три summon. Ascension sink залежить від rarity: Common коштує 3 shards, Rare/Epic — 2, Legendary — 3; duplicate rewards лишаються `1/2/3/5`.

Campaign stage більше не дорівнює одному миттєвому kill. Schema v7 зберігає `enemyIndex`: stages 1–200 мають три звичайні encounters, 201–1000 — чотири, далі — п'ять; boss-stage завжди містить одного боса. Engine сам визначає `stageCleared`, наступний encounter, HP і rewards. Tap gold нараховується лише з фактично знятого HP, тому overkill не створює валюту.
